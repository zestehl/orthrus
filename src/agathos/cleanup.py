...
Results are recorded in agathos.db for audit. Sessions are marked 'orphaned'
in agathos.db (not deleted — audit trail preserved).
"""
...
logger = logging.getLogger("agathos.cleanup")
...
def find_zombie_cron_sessions(
    ...
    """Find cron sessions in agathos.db referencing jobs that no longer exist."""
...
def mark_as_orphaned(
    ...
    """Mark a session as orphaned in agathos.db."""
    try:
        argus_cursor.execute(
            """
            UPDATE sessions
