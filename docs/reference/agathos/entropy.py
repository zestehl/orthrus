...
def detect_error_cascade(cursor: sqlite3.Cursor, session_id: str) -> List[Dict]:
    ...
    try:
        ...
        if max_run >= 3:
            ...
            # Dedup: skip if same or worse cascade already detected in last 10 min
            cursor.execute(
                """
                SELECT details FROM entropy_detections
                WHERE session_id = ? AND entropy_type = 'error_cascade'
            ...
            if existing:
                try:
                    prev = json.loads(existing["details"])
                    prev_errors = prev.get("consecutive_errors", 0)
                    if max_run <= prev_errors:
                        return detections
