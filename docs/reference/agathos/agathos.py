...
class Agathos:
    def __init__(self):
        ...
        # WAL deduplication: track last-emitted time per (session, entropy_type, key)
        # Prevents spam from repeated WAL events for the same pattern
        self._wal_dedup: Dict[str, float] = {}
            ...
            if source_type == "cron":
                ...
                for s in all_sessions:
                    ...
                    if source_type == "delegate" and src in ("delegate", "delegate_task"):
                        ...
                        # Skip ended sessions — no point enforcing directives on completed work
                        if s.get("ended_at"):
                            continue
                        sessions.append({
    ...
    def check_prime_directive(self, session_id: str) -> List[Dict]:
        ...
        # Skip sessions matching ignore patterns from directives.yaml
        import fnmatch
        for pattern in self._directives.get("ignore", []):
    ...
    def _is_session_active(self, session_id: str) -> bool:
        """Check if session is active and recently used.

        Only sessions with status='active' and recent activity get
        directive checks and corrective actions. Prevents Agathos from
        nuking historical/completed sessions with stale violations.

        Returns True if session qualifies for directive enforcement.
        """
        timeout_min = CONFIG.get("session_timeout_minutes", 60)
        try:
            self.cursor.execute(
                """
                SELECT status, last_activity_at FROM sessions
                WHERE session_id = ?
            """,
                (session_id,),
            )
            ...
            if last_activity:
                try:
                    ...
                    try:
                        ...
                        last_ts = _dt.fromisoformat(last_activity).timestamp()
            ...
            try:
                ...
                for session in sessions:
                    try:
                        ...
                        # Skip sessions already managed (restarted/killed)
                        self.cursor.execute(
                            "SELECT status, restart_count FROM sessions WHERE session_id = ?",
                            (sid,),
                        )
                        prev = self.cursor.fetchone()
                        if prev and prev["status"] in ("restarted", "killed"):
                            continue

                        self.register_session(session)
                        is_active = self._is_session_active(sid)
                        ...
                        # Persist non-WAL detections to DB (WAL detections are
                        # already persisted in _process_wal_events)
                        for det in entropy_detections:
                            if det.get("_source") != "wal":
                                self.cursor.execute(
                                    """
                        ...
                        # Check prime directives ONLY for active sessions
                        directive_checks = []
                        if (
                            is_active
                            and CONFIG.get("prime_directives_enabled", True)
                        ):
                            directive_checks = self.check_prime_directive(sid)

                        # Make and execute decision - if actions enabled
                        # Only act on active sessions not in ignore list
                        if is_active and CONFIG.get("actions_enabled", True):
                            decision = self.make_decision(
                                sid, entropy_detections, directive_checks
                            )
                            if decision:
                                # Re-check ignore before executing action
                                skip_action = False
                                import fnmatch as _fnmatch
                                for _pattern in self._directives.get("ignore", []):
                                    if _fnmatch.fnmatch(sid, _pattern):
                                        skip_action = True
                                        break
                                if not skip_action:
    ...
    def _process_wal_events(self):
        ...
        for event in events:
            if event.event_type == "repeat_detected":
                # Deduplicate: skip if same (session, tool) emitted within window
                dedup_key = f"repeat:{event.session_id}:{event.tool_name}"
                last = self._wal_dedup.get(dedup_key, 0)
                if now - last < dedup_window:
                    logger.debug(
                        "WAL: repeat tool '%s' in session %s (dedup, last %.0fs ago)",
                        event.tool_name,
                        event.session_id[:15],
                        now - last,
                    )
                    continue
                self._wal_dedup[dedup_key] = now
                ...
                # Deduplicate: skip if same (session, pattern) emitted within window
                pattern_key = ",".join(event.details.get("pattern", []))
                dedup_key = f"stuck:{event.session_id}:{pattern_key}"
                last = self._wal_dedup.get(dedup_key, 0)
                if now - last < dedup_window:
                    logger.debug(
                        "WAL: stuck loop in session %s (dedup, last %.0fs ago)",
                        event.session_id[:15],
                        now - last,
                    )
                    continue
