...
logger = logging.getLogger("orthrus.integration")
...
class HermesPlugin:
    ...
    def __init__(self, hermes_config: Optional[Dict[str, Any]] = None):
        ...
        self._monitor = None
        self._enabled = self.config.get("orthrus", {}).get("enabled", True)
...
def get_orthrus_version() -> Optional[str]:
    """Get Agathos version if installed."""
    try:
        import orthrus
        return getattr(orthrus, "__version__", "unknown")
    except ImportError:
        return None
