...
- DB file size tracking (state.db, orthrus.db, holographic_memory.db)
...
logger = logging.getLogger("orthrus.resources")
...
def _known_db_paths() -> List[str]:
    ...
    home = _hermes_home()
    argus_home = _orthrus_home()
    paths = [
        str(home / "state.db"),
        str(argus_home / "data" / "watcher" / "orthrus.db"),
    ]
...
def run_resource_check() -> Dict[str, Any]:
