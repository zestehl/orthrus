#!/usr/bin/env python3
"""
Deterministic audit script for 'argus' → 'orthrus' reference checks.

Scans the orthrus codebase for any remaining references to 'argus' that
may need updating to 'orthrus'. Does NOT auto-fix - only reports for review.

Usage:
    python orthrus_audit.py              # Full report
    python orthrus_audit.py --strict   # Exit error if issues found
    python orthrus_audit.py --json     # Machine-readable output
"""
...
class ArgusAuditor:
    """Audits orthrus codebase for stale 'argus' references."""

    # Files to skip (tests that intentionally check for stale refs, etc.)
    SKIP_FILES = {
        'orthrus_audit.py',  # This script
    ...
    SAFE_PATTERNS = [
        ...
        r'com\.hermes\.orthrus',  # Already correct
        r'orthrus',  # Already correct
    ...
    def categorize_and_score(self, match_text: str, line_text: str, file_path: Path) -> Tuple[str, str, str]:
        ...
        if re.search(r'from\s+argus\s+import|import\s+argus', line_text):
            return (
                'import',
                'critical',
                f"Change 'from argus import' to 'from orthrus import' or use relative imports"
            )
        ...
        if re.search(r'argus_\w+\s*\(', line_text):
            ...
            if func_name:
                ...
                new_name = old_name.replace('argus_', 'orthrus_')
                return (
                    'function_call',
                    'critical',
        ...
        if '.hermes/argus' in line_text or '.hermes" / "argus' in line_text:
            return (
                'path',
                'critical',
                "Change path from '.hermes/argus' to '.hermes/orthrus'"
            )
        ...
        if re.search(r'python\s+-m\s+argus', line_text):
            return (
                'module_reference',
                'critical',
                "Change 'python -m argus' to 'python -m orthrus'"
            )
        ...
        if re.search(r'\bargus_\w+\b', line_text) and not re.search(r'"[^"]*argus_\w+[^"]*"', line_text):
            ...
            if var_match:
                ...
                new_var = old_var.replace('argus_', 'orthrus_')
                return (
                    'variable',
                    'warning',
        ...
        if 'com.hermes.argus' in line_text:
            return (
                'launchd_label',
                'warning',
                "Change 'com.hermes.argus' to 'com.hermes.orthrus'"
            )
        ...
        if line_text.strip().startswith('#') or line_text.strip().startswith('"""') or line_text.strip().startswith("'''"):
            return (
                'comment',
                'info',
                "Review if comment/docstring should reference 'orthrus' instead of 'argus'"
            )
        ...
        if re.search(r'["\'][^"\']*argus[^"\']*["\']', line_text):
            return (
                'string_literal',
                'info',
                "Review if string should use 'orthrus' instead of 'argus'"
            )
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Constant) and isinstance(node.value, str):
                    if 'argus' in node.value.lower() and not self.is_safe_pattern(node.value):
                        ...
                        severity = 'warning'
                        suggestion = "Review if user-facing message should reference 'orthrus'"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Audit orthrus codebase for stale argus references')
    ...
    if args.root:
        ...
        # Default to orthrus/ directory relative to script
