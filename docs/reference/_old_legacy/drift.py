...
drift and records in orthrus.db for audit. Does NOT restart — running sessions
...
Monitored files:
  ...
  ~/.hermes/orthrus/directives.yaml — ARGUS prime directives
  orthrus/watcher_schema.sql  — AGATHOS DB schema
"""
...
logger = logging.getLogger("orthrus.drift")
...
def _monitored_files() -> List[Tuple[str, Path]]:
    ...
    files = [
        ...
        (".env", home / ".env"),
        ("directives.yaml", home / "orthrus" / "directives.yaml"),
    ]
    ...
    # watcher_schema.sql — relative to orthrus module
