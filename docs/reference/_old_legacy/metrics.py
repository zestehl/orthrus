...
Metrics exposed:
- orthrus_entropy_detections_total: Counter by entropy_type and severity
- orthrus_session_restarts_total: Counter by session_type
- orthrus_session_kills_total: Counter by session_type
- orthrus_provider_errors_total: Counter by provider and error_type
- orthrus_uptime_seconds: Gauge (process uptime)
"""
...
logger = logging.getLogger("orthrus.metrics")


class MetricsCollector:
    """Collect Agathos metrics for Prometheus or JSON export.

    Queries orthrus.db to aggregate metrics across multiple dimensions:
    ...
    Attributes:
        db_path: Path to orthrus.db
        start_time: Unix timestamp when collector was instantiated (for uptime)

    Usage:
        collector = MetricsCollector(Path("~/.hermes/orthrus/orthrus.db"))
    ...
    def collect_prometheus_metrics(self) -> str:
        ...
        Queries orthrus.db for recent metrics (last hour), formats as
        ...
        Metric names:
        - orthrus_entropy_detections_total (labels: type, severity)
        - orthrus_session_restarts_total (labels: session_type)
        - orthrus_session_kills_total (labels: session_type)
        - orthrus_uptime_seconds (no labels)

        Time window: Last 1 hour of data from orthrus.db.
        """
        lines = []

        # Counters
        lines.append("# HELP orthrus_entropy_detections_total Total entropy detections")
        lines.append("# TYPE orthrus_entropy_detections_total counter")
        for row in self._get_entropy_counts():
            lines.append(
                f'orthrus_entropy_detections_total{{type="{row["entropy_type"]}",severity="{row["severity"]}"}} {row["count"]}'
            )

        lines.append("# HELP orthrus_session_restarts_total Total session restarts")
        lines.append("# TYPE orthrus_session_restarts_total counter")
        for row in self._get_restart_counts():
            lines.append(
                f'orthrus_session_restarts_total{{session_type="{row["session_type"]}"}} {row["count"]}'
            )

        lines.append("# HELP orthrus_session_kills_total Total session kills")
        lines.append("# TYPE orthrus_session_kills_total counter")
        for row in self._get_kill_counts():
            lines.append(
                f'orthrus_session_kills_total{{session_type="{row["session_type"]}"}} {row["count"]}'
            )

        # Gauges
        lines.append("# HELP orthrus_uptime_seconds Agathos process uptime")
        lines.append("# TYPE orthrus_uptime_seconds gauge")
        lines.append(f"orthrus_uptime_seconds {time.time() - self.start_time}")
    ...
    def collect_json_metrics(self) -> Dict:
        ...
        Queries orthrus.db for recent metrics and returns as structured
...
def write_metrics_file(collector: MetricsCollector, output_path: Path) -> bool:
    ...
    Args:
        collector: MetricsCollector instance with orthrus.db connection
        output_path: File path to write metrics (e.g., /var/lib/node_exporter/textfile_collector/orthrus.prom)
    ...
    Example cron job:
        */5 * * * * orthrus metrics > /var/lib/node_exporter/textfile_collector/orthrus.prom
    """
    try:
