...
try:
    ...
    _logging.getLogger("orthrus").warning(
...
_DEFAULT_AGATHOS_CONFIG = {
    # Core paths
    "db_path": str(_orthrus_path("data", "watcher", "orthrus.db")),
    ...
    "ml_data_enabled": False,             # Export trajectories to ~/.hermes/orthrus/ml_data/
...
def _load_orthrus_config() -> Dict:
    """Load AGATHOS config from hermes config.yaml 'orthrus' key, falling back to defaults."""
    try:
        hermes_config = _hermes_load_config()
        orthrus_overrides = hermes_config.get("orthrus", {})
        return {**_DEFAULT_AGATHOS_CONFIG, **orthrus_overrides}
    except Exception:
        return dict(_DEFAULT_AGATHOS_CONFIG)


# Runtime config — loaded once at import
CONFIG = _load_orthrus_config()
...
logger = logging.getLogger("orthrus")
logger.setLevel(logging.INFO)


_rotating_handler = logging.handlers.RotatingFileHandler(
    str(Path(CONFIG["log_dir"]) / "orthrus.log"),
...
class Agathos:
    ...
    def _populate_tool_calls_from_session(self, session_id: str) -> None:
        """Read tool calls from state.db and insert into orthrus.db.tool_calls.
    ...
    def run(self):
        ...
        write_orthrus_pid_file()
        logger.info("ARGUS PID file written: %s", _get_orthrus_pid_path())
