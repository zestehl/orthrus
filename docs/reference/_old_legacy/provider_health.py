...
- orthrus.db tool_calls: error_message, success, duration_ms
- orthrus.db sessions: provider, model
...
logger = logging.getLogger("orthrus.provider_health")
...
def _collect_from_tool_calls(
    orthrus_cur: sqlite3.Cursor, since_iso: str
) -> Dict[str, List[Dict[str, Any]]]:
    """Pull failed tool calls from orthrus.db and classify by provider.
    ...
    try:
        ...
        for row in orthrus_cur.fetchall():
...
def run_provider_check(
    orthrus_cursor: sqlite3.Cursor,
    orthrus_conn: sqlite3.Connection,
) -> Dict[str, Any]:
    """Run a full provider health check and return aggregated report.

    Collects error data from both state.db (messages) and orthrus.db
    ...
    Args:
        orthrus_cursor: Database cursor for orthrus.db
        orthrus_conn: Database connection for commits
    ...
    Side effects:
        ...
        - Reads from orthrus.db (tool_calls, sessions)
        - Inserts into provider_health table
        - Commits orthrus_conn
    ...
    tc_errors = _collect_from_tool_calls(orthrus_cursor, since_iso)
    latencies = _collect_latencies(orthrus_cursor, since_iso)
